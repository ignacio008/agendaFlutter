...
class WidgetCreateNote extends StatefulWidget {
  const WidgetCreateNote({Key key}) : super(key: key);

  @override
  State<WidgetCreateNote> createState() => _WidgetCreateNoteState();
}

class _WidgetCreateNoteState extends State<WidgetCreateNote> {
  ...
  void _sendHtml(HtmlEditorController controller, BuildContext context)async {
      ...
      if(txt.isEmpty){
       ...
       if(titulo.isEmpty){
        showToasterror(erorTitulo);
        return;
      }
       if(descripcion.isEmpty){
        showToasterror(erorDescritpcion);
        return;
      }
 bool erroguardar=await QuerysService().SaveGeneralNota(reference: "note", id:idNote, collectionValues:ModelNote().toJsonBodyCrearNote(idNote,txt,hoy,titulo,descripcion),);
  if(erroguardar){
      ...
      Container(
        ...
        child: Column(
          ...
          children: const [
            ...
            Text("Felicidades se ha creado su nota", style:TextStyle(color: Colors.white)),
          ],
        ),
      );
